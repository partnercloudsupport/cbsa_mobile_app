...
class Task extends StatefulWidget {
  ...
  final int ids;
  final int id;
  Task({Key key, this.initials, this.fullName,this.id,this.ids}) : super(key: key);
...
class _TaskState extends State<Task> {
  ...
  buildListView(TaskModel model, int index) {
    DateTime date = DateTime.parse(model.tasksById[index]['duedate']);
        ...
        body: ScopedModel<TaskModel>(
          ...
          child: ScopedModelDescendant<TaskModel>(
            builder: (context, child, model) {
              ...
              return model.tasks.length==0? Center(child: Text('No Tasks Available',style: TextStyle(color: Colors.white,fontSize: 18.0),),) :
              Column(
                children: <Widget>[
