...
class UpdateFollowUp extends StatefulWidget {
  ...
  UpdateFollowUp({Key key, @required this.followup}) : super(key: key);

  @override
  _UpdateFollowUpState createState() => _UpdateFollowUpState();
}
...
class _UpdateFollowUpState extends State<UpdateFollowUp> {
  ...
  List<String> _hasPreparedSpaceList = [
    'Yes',
    'No',
    'Gave a future date',
    'No longer interested -hard',
  ...
  void initState() {
    ...
    if (widget.followup != null) {
      if (widget.followup.type != null) {
        _followUpType = widget.followup.type;
      }
      if (widget.followup.wasleadreached != null) {
        _followUpStatus = widget.followup.wasleadreached;
      }
      if (widget.followup.isinterested != null) {
        _isInterested = widget.followup.isinterested;
      }
      if (widget.followup.paystate != null) {
        _readyToPay = widget.followup.paystate;
      }
      if (widget.followup.spacepreparestate != null) {
        _hasPreparedSpace = widget.followup.spacepreparestate;
      }
      if (widget.followup.comment != null) {
        _comments = widget.followup.comment;
      }
      if (widget.followup.folupdate != null) {
        _followUpDate = widget.followup.folupdate;
        followUpDateController.text = DateFormat.yMMMMd()
            .format(DateTime.parse(widget.followup.folupdate));
      }
      print(widget.followup.inspectiondate);
      if (widget.followup.inspectiondate != null) {
        _siteInspectionDate = widget.followup.inspectiondate;
        siteInspectionDateController.text = DateFormat.yMMMMd()
            .format(DateTime.parse(widget.followup.inspectiondate));
      }
      if (widget.followup.installationdate != null) {
        _installationDate = widget.followup.installationdate;
        installationDateController.text = DateFormat.yMMMMd()
            .format(DateTime.parse(widget.followup.installationdate));
      }
    }
  }
            ...
            onPressed: () async {
              if (_formKey.currentState.validate()) {
                ...
                try {
                  ...
                  FollowUpService.updateFollowUp(followup, followup.id)
                      .then((response) {
                    // print(response.body);
                    ...
                    if (response.statusCode == 200) {
                      ...
                      model.updateFollowup(
                          FollowUps.map(decodedJson['followup']));
                  ...
                  Fluttertoast.showToast(
                      msg: 'There was a problem executing your request',
                      toastLength: Toast.LENGTH_SHORT);
                }
              }
            },
                  ...
                  Expanded(
                    ...
                    child: ListView(
                      children: <Widget>[
                          ...
                          child: Column(
                            children: <Widget>[
                              ...
                              Visibility(
                                visible: _readyToPay !=
                                        'No longer interested -hard' &&
                                    _readyToPay != 'No longer interested -soft',
                                child: Column(
                                  children: <Widget>[
                                    hasPreparedSpaceDropDown(),
                                    Visibility(
                                      visible: _hasPreparedSpace !=
                                              'No longer interested -hard' &&
                                          _hasPreparedSpace !=
                                              'No longer interested -soft',
                                      child: Column(
                                        children: <Widget>[
                                          Visibility(
                                            ...
                                            replacement: Column(
                                              children: <Widget>[
                                                siteInspectionDateTextField(
                                                    context),
                                                installationDateTextField(
                                                    context),
                                              ],
                                            ),
                                          ),
                        ...
                        //       Visibility(
                        //         visible:
                        //             _readyToPay == 'Ready now' ? true : false,
                        //         child: hasPreparedSpaceDropDown(),
                        ...
                        //           children: <Widget>[
                        //             Visibility(
                        //               visible: _hasPreparedSpace == 'No',
                        //               child: followUpDateTextField(context),
                        //             ),
                        //             Visibility(
                        //               visible: _hasPreparedSpace == 'Yes' &&
                        //                   _hasPreparedSpace == 'Yes',
                        //               child: Column(
                        //                 children: <Widget>[
